...
X_comb$Biome<-factor(X_comb$Biome,levels = c("Forests", 
                                              ...
                                              "Coral reefs",
                                              "Marine"))
tiff("Application fequency_Drones.tiff", unit="in",width = 6, height =6, res= 600,pointsize = 10)
ggplot(X_comb, aes(x = Biome, y = Subdiscipline, fill = Freq)) +
  geom_tile() +
  facet_wrap(~ Taxonomy) +
  scale_fill_gradient(low = "white", high = "blue", limits = c(0, 110), oob = scales::squish) +
  labs(title = "",
       x = "Ecosystem",
       y = "Subdiscipline",
       fill = "Frequency") +
...
tiff("Application fequency_Robots.tiff", unit="in",width = 6, height =6, res= 600,pointsize = 10)
ggplot(X_comb, aes(x = Biome, y = Subdiscipline, fill = Freq)) +
  geom_tile() +
  facet_wrap(~ Taxonomy) +
  scale_fill_gradient(low = "white", high = "blue", limits = c(0, 110), oob = scales::squish) +
...
tiff("Application fequency.tiff", unit="in",width = 6, height =6, res= 600,pointsize = 10)
ggplot(X_comb, aes(x = Biome, y = Subdiscipline, fill = Freq)) +
  geom_tile() +
  facet_wrap(~ Taxonomy) +
  scale_fill_gradient(low = "white", high = "blue", limits = c(0, 110), oob = scales::squish) +
...
tiff("Application fequency-Drones and Robots.tif", unit="in",width = 5.5, height =4, res= 600,pointsize = 10)
ggplot(XX_comb, aes(x = Biome, y = Subdiscipline, fill = Percentage)) +
  geom_tile() +
  facet_grid(Instrument ~ Taxonomy ) +
  scale_fill_gradient(low = "white", high = "#F16913", limits = c(0, 10), oob = scales::squish) +
  labs(title = "",
       x = "Ecosystem",
       y = "Scale",
       fill = "Percentage (%)") +
...
tiff("Fig. 4 and Fig. 5/Result/Tax_percentage.tif", unit="in",width = 2, height =3, res= 600,pointsize = 10)
...
ggplot(Tax_data, aes(x = reorder(Taxanomy, `Percentage (%)`), y = `Percentage (%)`)) +
  ylim(0,50)+
  geom_bar(stat = "identity", fill = "gray" ) + # Bar plot with specified values
